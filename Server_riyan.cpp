...
indahin ke satu class LogManajer

//pindah log ke local tiap client, gak global
vector<Log> logs;

//done
unordered_map<string, string> rfidToName;

//gabung ke class log konversi char sama string
struct LogRaw {
    char rfid[6];
    char time[21];
    char action[10];
...
//edit : cara baca biar bisa lnagung vector dan simpan data ke class User
...
//  		message += log.getRFID() + " " + log.getName() + " " + log.getTime() + " " + log.getAction() + "\n";
// 	}
...
// vector<Log> readLogsFromBinary(const string& filename) {
//     vector<Log> result;
...
//edit: jadi biar langung masuk vector logs, gak perlu lograw
void loadFileBinery(){
...
//edit jangan langung kirirm tpi jadi message aja dulu sekelian shorting -> listLogs()
...
// gabung searchlog sama sendsearching, juga gak langusng kirirm tapi pake message
// jadi lebih modular gak cuma nama, tapi bisa rfid/action/nama/ dan hariini
...
//edit biar gak error dan bisa pake vector
...
//ganti menu biar pake promt aja biar komunikasi server -client cuma string aja
...
// di jadii in class dan buat thread biar bisa multi client 
int main() {
    loadDatabaseFast();
    loadFileBinery();
    loadAction();
